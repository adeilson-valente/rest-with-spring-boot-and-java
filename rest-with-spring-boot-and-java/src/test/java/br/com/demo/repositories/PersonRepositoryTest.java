...
class PersonRepositoryTest extends AbstractIntegrationTest {
    ...
    void findPeopleByName() {
        ...
        person = repository.findPeopleByName("ilson", pageable).getContent().get(0);

        assertNotNull(person);
        assertNotNull(person.getId());
        assertEquals("Adeilson", person.getFirstName());
        assertEquals("Valente", person.getLastName());
        assertEquals("Male", person.getGender());
        assertNotNull("Campos Belos", person.getAddress());
